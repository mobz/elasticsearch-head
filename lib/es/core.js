(function() {
 ...
 eg PUT, "/twitter/tweet/1"
   ...
   visible columns and labels can be configured in _meta.visualConfig object like in example below.
        	...
        	"visualConfig" : {
            	...
            	"browser" 		  : {
                	"columns" 	: {
                		...
                		"date"		        : {"label":"Date"},
                		"size"		        : {"label":"Size"},
                		"_parent.folder"	: {"label":"Folder"},
                		"_parent"	        : {"label":"ParentId"}
                	}
                }
            }
        }
...
Converter for columns can access to folowing variables: 
  prop - name of property
  , spec, path, column, hit){
  ...
  */
    ...
    getVisualConfig: function(index,type){
      var visual = this.indices[index].visualConfigs[type];
    ...
    isLoadParents: function(index,type){
      var visual = this.getVisualConfig(index,type);
      if(visual != null && typeof visual.loadParents != "undefined"){
        return visual.loadParents;
      }else{
        return false;
      }
    },
    getBrowserConfig: function(index,type){
      var visual = this.getVisualConfig(index,type);
      if(visual != null && typeof visual.browser != "undefined"){
        return visual.browser;
      }else{
        return null;
      }
    },
    getBrowserColumns: function(index,type){
      var browser = this.getBrowserConfig(index,type);
      if(browser != null && typeof browser.columns != "undefined"){
        return browser.columns;
      }else{
        return null;
      }
    },
    getColumnConverters: function(index,type){
      var columnConverters = this.indices[index].columnConverters[type];
      if(typeof columnConverters == "undefined"){
        columnConverters = {};
        this.indices[index].columnConverters[type] = columnConverters;
        var visual = this.getVisualConfig(index,type);
        if(visual != null && typeof visual.columnConverters != "undefined"){
          var func, converter;
          for(var prop in visual.columnConverters){
            converter = new ColumnConverter();
            try{
              //build converter method
              func = "(function(hit, column){" + visual.columnConverters[prop]+"})";
              converter.convert = eval(func);
            } catch (e){
            	if("console" in window) {
						    console.log("Error to compile converter: "+ func+ ", exception: "+ e);
					    }
            }
            columnConverters[prop] = converter;
          }
        }
      }
    ...
    getParentMapping: function(index,type){
      if(this.indices[index].parents[type] != "undefined"){
        return this.indices[index].parents[type];
      }else{
        return null;
      }
    },
		refresh: function(state) {
			...
			for(var index in state.metadata.indices) {
				indices[index] = { types: [], fields: {}, paths: {}, parents: {}, visualConfigs: {}, columnConverters: {} };
          ...
          if (typeof mapping._meta != "undefined" && typeof mapping._meta.visualConfig!= "undefined"){
            indices[index].visualConfigs[type] = mapping._meta.visualConfig;
          }
				}
			}
    ...
    getParent: function(hit){
      if (typeof hit.fields != "undefined" && typeof hit.fields._parent != "undefined"){
        return  hit.fields._parent;
      }else{
        return null;
      }
    },
        ...
        if(metadata.isLoadParents(hit._index, hit._type)){
          ...
          if (parent){
            ...
            if (typeof indexToTypeToParentIds[hit._index] == "undefined"){
             ...
             if (typeof indexToTypeToParentIds[hit._index][hit._type][parent] == "undefined"){
    ...
    buildMapByIndexTypeIdDoc: function(docs){
      ...
      docs.forEach(function(doc) {
        if (typeof indexTypeParentIdDoc[doc._index] == "undefined"){
         indexTypeParentIdDoc[doc._index] = {};
        }

        if (typeof indexTypeParentIdDoc[doc._index][doc._type] == "undefined"){
    ...
    _load_parents: function(query, res) {
      ...
      var fillRecursive = function(parentPath, spec, row) {
          ...
          if(prop != "_source" && prop != "fields"){
            ...
            if(typeof columnToPath[path] == "undefined"){
              columnToPath[path] = path;
              columns.push(path);
            }
            row[path] = (spec[prop] === null ? "null" : spec[prop]).toString();
					}
				}
			};
      ...
      var defaultColumnConverter = new ColumnConverter();

      this.data = res.hits.hits.map(function(hit) {
				var row = { _source: hit};
        var columnDefs = metadata.getBrowserColumns(hit._index, hit._type);
        var converters =  metadata.getColumnConverters(hit._index, hit._type);
        if(columnDefs){
          for(var name in columnDefs){
            var column = columnDefs[name];
            var converter = defaultColumnConverter;
            if(typeof column.converter != "undefined" 
               && typeof converters[column.converter] != "undefined"){
   				    converter = converters[column.converter];
            }
            var path = column.path.replace("_source.","");
            path = path.replace("fields.","");
            if(!columns.contains(name)){
              columnToPath[name] = path;
              columns.push(name);
            }
            row[name] = converter.convert(hit, column);
          }
        }else{
...
function ColumnConverter() {
  this.resolveProperty = function(hit, path) {
    var relObj=hit;
    var parts = path.split(".");
    for(var i =0;i<parts.length;i++){
      try{
  ...
  this.convert = function(hit, column){
    return this.resolveProperty(hit, column.path);
  }
}
