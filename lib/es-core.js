   ...
   notes
	...
	es.MetaData = acx.ux.Observable.extend({
		...
		refresh: function(state) {
			// will not handle multiple paths that reference the same field, or multiple indexes with the same types (at the moment)
				...
				(function(prop, path) {
					for(var n in prop) {
						if("properties" in prop[n]) {
							...
							listeners.forEach(function(obj) { obj[field.index_name] = field; });
						}
					}
				})(properties, []);
			}
	...
	es.QueryDataSourceInterface = acx.data.DataSourceInterface.extend({
		defaults: {
			metadata: null, // (required) instance of es.MetaData, the cluster metadata
		...
		_results_handler: function(query, res) {
			...
			this.sort = query.search.sort[0] || { column: "_score", dir: "desc" };
				...
				var row = (function(path, spec, row) {
					for(var prop in spec) {
						if(acx.isObject(spec[prop])) {
							...
							row[field_name] = spec[prop].toString();
						}
					}
					return row;
