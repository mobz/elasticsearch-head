(function() {
	...
	es.ui.QueryFilter = acx.ui.Widget.extend({
		...
		_openFilter_handler: function(jEv) {
			var section = $(jEv.target).closest(".sidebar-section"),
					field_name = section.children(".sidebar-subhead").text(),
					body = section.children(".sidebar-subbody");
			if(! body.data("loaded")) {
				var spec = this.metadata.fields[field_name];
				if(spec.core_type === "string") {
					body.append(this._textFilter_template(spec));
				}
				body.data("loaded", true);
			}
			body.slideToggle(200, function() { body.find("INPUT").focus(); });
		},
		...
		_filters_template: function() {
			return { tag: "DIV", cls: "section queryFilter-filters", children: [
				...
				{ tag: "DIV", children: acx.eachMap(this.metadata.fields, function(name, data) {
					return { tag: "DIV", cls: "sidebar-section", children: [
						{ tag: "DIV", cls: "sidebar-subhead", onclick: this._openFilter_handler, text: name },
						{ tag: "DIV", cls: "sidebar-subbody" }
					] };
				}, this ) }
			] };
		},
