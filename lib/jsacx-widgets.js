...
acx.ui.Table = acx.ui.Widget.extend({
	...
	_scroll_handler: function(jEv) {
		var body = $(jEv.target);
		this.el.find(".uiTable-headers").scrollLeft(body.scrollLeft());
	},
	_dataClick_handler: function(jEv) {
		var row = $(jEv.target).closest("TR");
		if(row.length) {
			this.fire("rowClick", this, { row: row.data("row") } );
		}
	},
	...
	_headerRow_template: function(columns) {
		return { tag: "TR", cls: "uiTable-header-row", children: columns.map(function(column) {
			var sort = (this.config.store.sort.column === column) ? " uiTable-sort" : "";
			return { tag: "TH", cls: "uiTable-header-cell" + sort, children: [
				{ tag: "DIV", children: [
					{ tag: "DIV", cls: "uiTable-headercell-menu", text: sort && this.config.store.sort.dir === "asc" ? "\u25b2" : "\u25bc" },
