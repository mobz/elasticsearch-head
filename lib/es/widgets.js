...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = acx.ui.Widget.extend({
		...
		_createAliasAction_handler: function(index) {
			console.log(index);
			var fields = new acx.ux.FieldCollection({
				fields: [
			...
			return index.name ? { tag: "TH", cls: (closed ? "close" : ""), children: [
				...
				{ tag: "DIV", children: [
					...
					new acx.ui.MenuButton({
						...
						menu: new acx.ui.MenuPanel({
							items: [
								{ text: "New Alias", onclick: function() { this._createAliasAction_handler(index)}.bind(this) },
					...
					return {
						...
						children: [
							{	tag: 'SPAN',
								...
								onclick: function() {
									if(prompt("Type 'REMOVE' to remove alias " + alias.name + ". There is no undo!") === "REMOVE") {
										return alert('not implemented yet, because I don\'t know how to use the index object here.');
