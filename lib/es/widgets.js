	...
	es.ui.ClusterOverview = es.ui.Page.extend({
		...
		init: function() {
			...
			this._refreshButton = new acx.ui.SplitButton({
				label: acx.text("General.RefreshResults"),
				items: [
					{ label: "Manual Refresh", value: -1, selected: true },
					{ label: "Refresh as fast as possible", value: 0 },
					{ label: "Refresh eveny 5 seconds", value: 5 },
					{ label: "Refresh eveny minute", value: 60 }
				],
				onselect: function( btn, event ) {
					this._redrawValue = event.value * 1000;
					if( event.value < 0 ) {
		...
		redraw: function(command) {
			if(command === "reset") {
				this._resetTimer && window.clearTimeout( this._resetTimer );
