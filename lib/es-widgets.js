(function() {
	...
	es.AnyQuery = es.AbstractQuery.extend({
		...
		_request_handler: function() {
			this.el.find("PRE.es-out").text("searching...");
			this._super({
	...
	es.StructuredQuery = es.AbstractQuery.extend({
		...
		_indexChanged_handler: function(index) {
			...
			this.filter = new es.FilterBrowser({
				base_uri: this.config.base_uri,
				index: index,
				onStaringSearch: function() { this.el.find("PRE.es-out").text("Searching..."); }.bind(this),
		...
		_results_handler: function(results) {
			this.el.find("PRE.es-out").text(JSON.stringify(results, null, "    "));
		},
		...
		_main_template: function() {
			return { tag: "DIV", children: [
				...
				{ tag: "DIV", child: { tag: "PRE", cls: "es-out" } }
			]};
		}
	});
