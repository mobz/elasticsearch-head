         ...
         ....
{
  ...
  */
    ...
    _load_parents: function(query, res) {
      ...
      var fillColumns = function(columns, deep, parentPath, spec, row) {
			 for(var prop in spec) {
          var path = parentPath;
          if(deep != 0 || (prop != "_source" && prop != "fields")){
            if(path.length>0){
               path = path.concat(".");
            }
            path = path.concat(prop);
					...
					if(acx.isObject(spec[prop])) {
						arguments.callee(columns, deep+1, path, spec[prop], row);
					} else if(acx.isArray(spec[prop])) {
						if(spec[prop].length) {
							arguments.callee(columns, deep+1, path, spec[prop][0], row)
						}
					} else{
            var columnName=path;
            if(columns!=null){
              var column = columns[path];
              if(typeof column != "undefined"){
                if(typeof column.label != "undefined"){
                  columnName = column.label;
                }
                columnToPath[columnName] = path;
   							row[columnName] = (spec[prop] === null ? "null" : spec[prop]).toString();
              }
            }else{
              columnToPath[columnName] = columnName;
							row[columnName] = (spec[prop] === null ? "null" : spec[prop]).toString();
            }
					}
				}
      ...
      this.data = res.hits.hits.map(function(hit) {
				var row = { _source: hit};
        fillColumns(metadata.getBrowserColumns(hit._index, hit._type), 0, "", hit, row);
				return row;
			}, this);
