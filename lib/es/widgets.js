(function() {
  ...
  window.isUnset = function (object) {
	...
	es.ui.HtmlPreview = acx.ui.InfoPanel.extend({
		defaults: {
			data: null, // (required)
			modal: false,
			open: true,
		...
		_body_template: function() {
			body = this._super();
			if(isUnset(this.config.data.isHTML)){
				body.child = { tag: "DIV", text: this.config.data  };
			}else{
				if(this.config.data.isHTML){
					body.child = { tag: "DIV", html: this.config.data.value  };
				}else{
					body.child = { tag: "DIV", text: this.config.data.value  };
				}
			}
			return body;
		}
	...
	es.ui.Table = acx.ui.Table.extend({
		...
		showPreview: function(row) {
			...
			if(isUnset(rowData._preview)){
				this.preview = new es.ui.JsonPanel({
					title: acx.text("Browser.ResultSourcePanelTitle"),
					json: rowData._source,
					onClose: function() { row.removeClass("selected"); }
				});
			}else {
				this.preview = new es.ui.HtmlPreview({
					title: rowData._preview.converter.title,
					data: rowData._preview.convert(rowData._source),
					onClose: function() { row.removeClass("selected"); }
				});
			}
		},
	...
	es.ui.Browser = es.ui.Page.extend({
		...
		init: function() {
          ...
          this.query = new es.Query( { cluster: this.cluster, metadata: metadata } );
...
//		path: "",		 		// (required) path to request
	...
	es.StructuredQuery = es.AbstractQuery.extend({
		...
		_tableResults_handler: function(results, metadata) {
			...
			var qdi = new es.QueryDataSourceInterface( { metadata: metadata, query: new es.Query({ metadata: metadata }) });
