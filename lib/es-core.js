   ...
   creates
	...
	es.QueryDataSourceInterface = acx.data.DataSourceInterface.extend({
		...
		_results_handler: function(query, res) {
			this.meta = { total: res.hits.total, shards: res._shards };
			console.log("QueryDataSourceInterface._results_handler", this.meta)
			...
			this.data = res.hits.hits.map(function(hit) {
				return this.columns.reduce(function(row, column) {
					try {
