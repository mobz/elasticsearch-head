(function() {
	...
	es.ui.QueryFilter = acx.ui.Widget.extend({
		...
		_dateFilterChange_handler: function(jEv) {
			...
			var range = window.dateRangeParser.parse(jEl.val());
			...
			jEl.data("uqid", this.query.addClause( value, spec.field_name, "range", "must"));
			this.requestUpdate(jEv);
		},
		...
		_dateFilter_template: function(spec) {
			return { tag: "INPUT", data: { spec: spec }, onKeyup: this._dateFilterChange_handler };
		}
	});
