...
*/
  ...
  es.Cluster = acx.Class.extend({
    ...
    getBrowserPreviewConverter: function(index,type){
      var parent = this.getBrowserConfig(index,type);
      return parent === null || isUnset(parent.previewConverter)?null:parent.previewConverter;
    },
	...
	es.QueryDataSourceInterface = es.AbstractDataSourceInterface.extend({
      ...
      if(this.columnToPath){
		...
		_getData:function(query,res,metadata) {
			...
			this.data=res.hits.hits.map(function(hit) {
				var row={_source:hit};
				var previewConverter=metadata.getBrowserPreviewConverter(hit._index,hit._type);
				if(previewConverter && !isUnset(es.converter[previewConverter.id])){
					row._preview = {convert:es.converter[previewConverter.id], converter:previewConverter};
				}
				var columnDefs=metadata.getBrowserColumns(hit._index,hit._type);
				if(columnDefs){
					columnDefs.forEach(function(column) {
						var converter=isUnset(column.converter)
								||isUnset(es.converter[column.converter.id])?"value"
								:column.converter.id;
