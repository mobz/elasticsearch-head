   ...
   notes
	...
	es.MetaData = acx.ux.Observable.extend({
		...
		refresh: function(state) {
			...
			function createField(mapping, index, type, path, name, isSubField) {
				// Deal with undefined isSubField
				isSubField = new Boolean(isSubField === "false" ? false : isSubField);
				...
				if (field.type === "multi_field" && typeof field.fields !== "undefined") {
					for (var subField in field.fields) {
						// var name = field_name + "." + subField;
						// if (subField == field.field_name) {
						// name = field_name
						// }
						field.fields[subField] = createField(field.fields[subField], index, type, path.concat(subField), name + "." + subField, true);
					}
				}
