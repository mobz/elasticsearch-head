...
//		data: null			(required)	
	...
	es.ui.DateHistogram = acx.ui.Widget.extend({
		defaults: {
			printEl: null, // (optional) if supplied, clicking on elements in the histogram changes the val of the printEl
			cluster: null, // (required)
		...
		init: function() {
			...
			this.config.query.on("setIndex", function(query, params) {
				...
				query.on("results", this._initHistogram_handler);
			}.bind(this));
			...
			this._initHistogram_handler();
		},
		_initHistogram_handler: function(query) {
			query && query.removeObserver(arguments.callee);
