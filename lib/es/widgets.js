(function() {
	...
	es.ui.HelpPanel = acx.ui.DraggablePanel.extend({
		defaults: {
			ref: "",
			open: true,
			autoRemove: true,
	...
	es.ui.JsonPanel = acx.ui.DraggablePanel.extend({
		defaults: {
			json: null, // (required)
			modal: false,
			open: true,
	...
	es.ui.Table = acx.ui.Table.extend({
		...
		appendTo: function(parent) {
			if(parent) {
				var height = parent.height() || ( $(document).height() - parent.offset().top - 45 );
		...
		_showPreview_handler: function(obj, data) {
			...
			this.preview = new es.ui.JsonPanel({
				...
				json: data.row.data("row")._source,
		...
		_main_template: function() {
			return { tag: "DIV", cls: "anyRequest", children: [
				{ tag: "DIV", cls: "anyRequest-request", children: [
					...
					new es.ui.SidebarSection({
						title: acx.text("AnyRequest.GraphResult"),
						help: "AnyRequest.GraphHelp",
						body: { tag: "DIV", children: [
...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = acx.ui.Widget.extend({
		defaults: {
...
//				this.cluster.get("_cluster/nodes/stats", this._clusterNodeStats_handler);
			} else if(this.status && this.clusterState) {
				...
				function getIndexForIndex(routings, i) {
					var index = indexIndices[i] = (i in indexIndices) ?
							...
							: routings.push( newIndex(i) ) - 1;
					indices[index] = i;
					return index;
				}
				acx.each(this.status.indices, function(name, index){
					acx.each(index.shards, function(name, shard) {
						shard.forEach(function(replica){
							var node = replica.routing.node;
							var index = replica.routing.index;
							var shard = replica.routing.shard;
				...
				this.el.empty().append(this._cluster_template(cluster, indices))
			}
		},
		...
		_replica_template: function(replica) {
			return { tag: "DIV",
				cls: "clusterOverview-replica" + (replica.routing.primary ? " primary" : "") + ( " state-" + replica.routing.state ),
				text: replica.routing.shard.toString()
			};
		},
		_routing_template: function(routing) {
			var cell = { tag: "TD", cls: "clusterOverview-routing", children: [] };
		...
		_indexHeader_template: function(index) {
			return index.name ? { tag: "TH", children: [
				{ tag: "DIV", cls: "clusterOverview-title", text: index.name },
				{ tag: "DIV", text: "size: " + index.status.index.primary_size + " (" + index.status.index.size + ")" },
				{ tag: "DIV", text: "docs: " + index.status.docs.num_docs + " (" + index.status.docs.max_doc + ")" },
				{ tag: "DIV", children: [
		...
		_main_template: function() {
			return { tag: "DIV", id: this.id(), cls: "clusterOverview" };
		}
	});
...
//		data: null			(required)	
	...
	es.ElasticSearchHead = acx.ui.Widget.extend({
		...
		_openClusterOverview_handler: function(jEv) { this.open(new es.ui.ClusterOverview({ cluster: this.cluster }), jEv); },
			...
			return { tag: "DIV", cls: "es", children: [
				{ tag: "DIV", id: this.id("header"), cls: "es-header", children: [
					...
					{ tag: "DIV", cls: "es-header-menu", children: [
						...
						{ tag: "DIV", cls: "es-header-menu-item es-right", text: "Overview", onclick: this._openClusterOverview_handler },
