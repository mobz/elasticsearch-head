(function( es, acx, raphael ) {
	...
	es.ui.Table = acx.ui.Table.extend({
		...
		_showDeleteMenu_handler: function(obj, data) {
			...
			var menuPanel = new acx.ui.MenuPanel({
				items: [
					{ text: acx.text("Browser.DeleteRecord"), onclick: function() { this._deleteRow(data.row) }.bind(this) }
				]
			});
		...
		_deleteRow: function(row) {
			var indexCell = row.find(":nth-child(1)").find("div");
			var typeCell = row.find(":nth-child(2)").find("div");
			var idCell = row.find(":nth-child(3)").find("div");
			// Delete from the index.
			this.cluster.delete(indexCell.text() + "/" + typeCell.text() + "/" + encodeURIComponent(idCell.text()));
