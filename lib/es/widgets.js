	...
	es.ui.AnyRequest = es.ui.Page.extend({
		...
		_responseError_handler: function (response) {
			...
			try {
				...
				if (obj) this._responseWriter_handler(obj);
			} catch (err) {
			}
		},
...
//		path: "",		 		// (required) path to request
	...
	es.ElasticSearchHead = acx.ui.Widget.extend({
		...
		showNew: function(type, config, jEv, tab_text) {
			...
			// Add the tab and its click handler
			...
			return this.show(type, config, jEv);
		},

		_openAnyRequest_handler: function(jEv) { this.show("AnyRequest", { cluster: this.cluster }, jEv); },
		_openNewAnyRequest_handler: function(jEv) { this.showNew("AnyRequest", { cluster: this.cluster }, jEv, acx.text("Nav.AnyRequest")); },
			...
			return { tag: "DIV", cls: "es", children: [
				{ tag: "DIV", id: this.id("header"), cls: "es-header", children: [
					...
					{ tag: "DIV", cls: "es-header-menu", children: [
						...
						{ tag: "DIV", cls: "es-header-menu-item es-left", text: acx.text("Nav.StructuredQuery"), onclick: this._openStructuredQuery_handler },
		...
		newTab: function(text, events) {
			var $el = $('<div class="es-header-menu-item es-left"></div>')
				.text(text);
			var $close = $('<a> [-]</a>').appendTo($el);
