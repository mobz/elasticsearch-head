 ...
 * Eugen Eisler
 *
 * namespace for column and preview converters is "window.es.converter"
 * 
 * Column Converters:
 * 	custom column converter can be added with this signature: 
 * 	converter.<id of converter>(hit,column)
 *  converted value of column can be just a target value or object with two properties {value:xx, isHTML:true/false}
 ...
 *
 * Preview/Type Converter:
 * 	custom preview converter can be added with this signature: 
 * 	converter.<id of converter>(hit,previewConverter)
 *  converted value of column can be just a target value or object with two properties {value:xx, isHTML:true/false}
 *	if isHTML is true then the value is rendered as innerHTML of DIV element of body.child element
 *  if isHTML is true then as text
 *	converter can be bind to columns in "_meta.elasticsearch-head.browser.previewConverter" by "{ "id": "fileInfo", "title":"File Information"}
 * 
            	...
            	"browser" 		: {
                	"columns" : [
                		...
                		{"label": "ContainerId", "path":"fields._parent"}
									...
									"previewConverter"	:	{	"id": "fileInfo", "title":"File" }
                }
            }
        }
    }
...
(function() {
	...
	//preview converters
	converter.fileInfo=function(hit,previewColumn) {
		var ret = hit._id;
		return {value:ret,isHTML:true};
	};
