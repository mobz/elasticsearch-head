...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = acx.ui.Widget.extend({
		...
		_postIndexAction_handler: function(action, index) {
			...
			return index.name ? { tag: "TH", cls: (closed ? "close" : ""), children: [
				...
				{ tag: "DIV", children: [
					...
					new acx.ui.MenuButton({
						...
						menu: new acx.ui.MenuPanel({
							items: [
								{ text: closed ? "Open" : "Close", onclick: function() { this._postIndexAction_handler(closed ? "_open" : "_close", index) }.bind(this) },
