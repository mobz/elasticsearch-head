(function() {
	...
	es.BoolQuery = acx.ux.Observable.extend({
		...
		addClause: function(value, field, op, bool) {
			...
			this.search.query.bool[bool].push(opo);
			this.refmap[uqid] = { clause: opo, bool: this.search.query.bool[bool] };
		...
		removeClause: function(uqid) {
			...
			ref.bool.splice(ref.bool.indexOf(ref.clause), 1);
		},
		getData: function() {
...
//							req.facets[field + "_stat"] = {"terms":{"field":field,"size":50}}; break; DISBALED (performance reasons)
					...
					if(val.length) {
						if(meta.core_type === "string") {
							val.split(/\s+/).forEach(function(term) {
								if(term) {
									search.addClause(term, meta.index_name, "prefix", "must");
								}
							});
						}
					}
		...
		_metaFilter_templates: {
			"boolean" : function(meta, onChange) {
				return { tag: "SELECT", onchange: onChange, children: ["-select-","true","false"].map(acx.ut.option_template) };
			},
