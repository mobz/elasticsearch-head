...
//				this.cluster.get("_cluster/nodes/stats", this._clusterNodeStats_handler);
				...
				acx.each(this.clusterState.routing_table.indices, function(name, index){
				...
				acx.each(status.indices, function(name, index) {
						index.aliases.forEach(function(alias) {
							var aliasIndex = aliasesIndex[alias] = (alias in aliasesIndex) ? aliasesIndex[alias] : aliases.push( { name: alias, indices: [].concat(indexClone) }) - 1;
							var indexIndex = indexIndices[name];
							console.log(aliasIndex, indexIndex);
							aliases[aliasIndex].indices[indexIndex] = true;
						});
				});
		...
		_indexHeader_template: function(index) {
			return index.name ? { tag: "TH", children: [
				...
				{ tag: "DIV", text: "size: " + (index.status ? index.status.index.primary_size + " (" + index.status.index.size + ")" : "unknown" ) },
		...
		_alias_template: function(alias) {
			console.log("_alias_template", alias);
			return { tag: "TR", children: [ { tag: "TD" } ].concat(alias.indices.map(function(index) {
				return { tag: "TD", cls: index ? "clusterOverview-hasAlias" : "", text: index ? alias.name : "" };
			}, this)) };
		},
