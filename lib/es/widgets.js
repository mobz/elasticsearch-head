...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = acx.ui.Widget.extend({
		...
		redraw: function(command) {
			if(command === "reset") {
				...
				acx.each(this.clusterState.metadata.indices, function(name, index) {
	...
	es.ElasticSearchHead = acx.ui.Widget.extend({
		...
		_openClusterNodes_handler: function(jEv) { this.open(new es.ui.SimpleGetQuery({ cluster: this.cluster, path: "_cluster/nodes" }), jEv); },
