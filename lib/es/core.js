(function() {
 ...
 eg PUT, "/twitter/tweet/1"
   ...
   If the object is not defined then all meta and _source fields are displayed 
         ....
      "_meta" : {
            "visualConfig" : {
            	"loadParents"  	: true,
            	"browser" 		: {
                	"columns" 		: {
                		"_source.name" 				  : {"label":"Name"},
                		"_source.fileType"		  : {"label":"Type"},
                		"_source.date"				  : {"label":"Date"},
                		"_source.size"				  : {"label":"Size"},
                		"_parent._source.folder"	: {"label":"Folder"}
                	}
                }
            }
        } 
    ...
    loadParents: function(res){
      ...
      if(this.columnToPath){
        pathIndex = this.columnToPath[index].replace("_source.","");
      }
	...
	es.QueryDataSourceInterface = es.AbstractDataSourceInterface.extend({
      ...
      if(this.columnToPath){
        for(var prop in this.columnToPath){
          ...
          if(path.replace("_source.","") == sortField){
            sortField = prop;
          }
        }
      }
    ...
    _load_parents: function(query, res) {
		...
		_getData: function(query, res, metadata) {
		  var columnToPath = this.columnToPath = {};
      var fillColumns = function(columns, parentPath, spec, row) {
			 for(var prop in spec) {
					if(acx.isObject(spec[prop])) {
						arguments.callee(columns, parentPath.concat(prop), spec[prop], row);
					} else if(acx.isArray(spec[prop])) {
						if(spec[prop].length) {
							arguments.callee(columns, parentPath.concat(prop), spec[prop][0], row)
						}
					} else{
						var dpath = parentPath.concat(prop).join(".");
            var columnName=dpath;
            if(columns!=null){
              var column = columns[dpath];
              if(typeof column != "undefined"){
                if(typeof column.label != "undefined"){
                  columnName = column.label;
                }
                columnToPath[columnName] = dpath;
   							row[columnName] = (spec[prop] === null ? "null" : spec[prop]).toString();
              }
            }else{
              columnName = columnName.replace("_source.","");
              columnToPath[columnName] = dpath;
							row[columnName] = (spec[prop] === null ? "null" : spec[prop]).toString();
            }
					}
      ...
      this.data = res.hits.hits.map(function(hit) {
				var row = { _source: hit};
        fillColumns(metadata.getBrowserColumns(hit._index, hit._type), [], hit, row);
				return row;
			}, this);
      query.columnToPath = columnToPath;
      this.columns = Object.keys(columnToPath);
   }
	});
