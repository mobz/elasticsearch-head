(function() {
	...
	es.JsonPretty = acx.ui.Widget.extend({
		...
		pretty: { // from https://github.com/RyanAmos/Pretty-JSON/blob/master/pretty_json.js
			"expando" : function(value) {
				return (/array|object/i).test(value.constructor.name) ? "expando" : "";
			},
	...
	es.TableResults = acx.ui.Widget.extend({
		defaults: {
			results: {} // (required) elastic search results object
		},
		init: function(parent) {
			this._super();
			this.el = $(this._main_template());
		...
		_main_template: function() {
			var res = this.config.results, hits = res.hits.hits;
			var headers = (function(path, spec, headers) {
			...
			return { tag: "DIV", cls: "es-tableResults", children: [
				{ tag: "DIV", text: "Searched " + res._shards.successful + " of " + res._shards.total + " shards. " + res.hits.total + " hits" },
				{ tag: "TABLE", cls: "tableResults", children: [
					this._headers_template(headers),
					this._body_template(headers, hits)
				] }
			]};
		},
	...
	es.ElasticSearchHead = acx.ui.Widget.extend({
		...
		_openBrowser_handler: function(jEv) { this.open(new es.Browser({ base_uri: this.config.base_uri }), jEv);  },
		...
		_openClusterNodesStats_handler: function(jEv) { this.open(new es.SimpleGetQuery({ base_uri: this.config.base_uri, path: "_cluster/nodes/stats" }), jEv); },
		_openStatus_handler: function(jEv) { this.open(new es.SimpleGetQuery({ base_uri: this.config.base_uri, path: "_status" }), jEv); },
		_openInfo_handler: function(jEv) { this.open(new es.SimpleGetQuery({ base_uri: this.config.base_uri, path: "" }), jEv); },
			...
			return { tag: "DIV", cls: "es", children: [
				{ tag: "DIV", id: this.id("header"), cls: "es-header", children: [
					{ tag: "DIV", cls: "es-header-top", children: [
						new es.Cluster({ base_uri: this.config.base_uri, onStatus: this._status_handler }),
