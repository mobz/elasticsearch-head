	...
	es.FilterBrowser = es.AbstractQuery.extend({
		...
		_search_handler: function() {
			...
			this.filtersEl.find(".es-filterBrowser-row").each(function(i, row) {
				...
				if(what === "match_all") {
					...
					var op = row.find(".es-op").val(), opo = {}, qual;
					if(op === "range") {
						var lowqual = row.find(".es-lowqual").val(), highqual = row.find(".es-highqual").val();
						qual = {};
						if(lowqual.length) qual[row.find(".es-lowop").val()] = lowqual;
						if(highqual.length) qual[row.find(".es-highop").val()] = highqual;
					} else {
						qual = row.find(".es-qual").val();
					}
					var whato = {}; whato[what] = qual; opo[op] = whato;
					bool.push(opo);
				}
			});
		...
		_changeQueryField_handler: function(jEv) {
			...
			if(spec.type === 'match_all') {
				...
				ops = ["term", "wildcard", "prefix", "fuzzy", "range"];
		...
		_changeQueryOp_handler: function(jEv) {
			...
			if(opv === 'term' || opv === 'wildcard' || opv === 'prefix' || opv === 'fuzzy') {
